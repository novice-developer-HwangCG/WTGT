...
void loop() {
  ...
  const int ADC_MIN_FOR_MOVE = (MIN_PWM * ADC_MAX) / 255;  // ≈1606
  ...
  if (enable == 0) {
    ...
    if (raw <= ADC_MIN_FOR_MOVE) {
      // 이 영역에서는 모터 정지
      pwm = 0;
    } else {
      int adc_span = ADC_MAX - ADC_MIN_FOR_MOVE;
      int adc_eff  = raw - ADC_MIN_FOR_MOVE;

      pwm = MIN_PWM + (adc_eff * (255 - MIN_PWM)) / adc_span;
    }
  }
